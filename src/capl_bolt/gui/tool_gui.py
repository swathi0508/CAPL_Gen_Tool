...
class CaplGenGUI(QMainWindow):
    ...
    def _build_input_section(self):
        ...
        btn_req = QPushButton("BROWSE")
        btn_req.setObjectName("util_btn")
        btn_req.clicked.connect(self._browse_file)
        inputs_vbox.addWidget(
            self._create_input_row("Input Requirements Sheet (.xlsx):", self.req_path_edit, btn_req)
        )

        self.arxml_path_edit = QLineEdit()
        self.arxml_path_edit.textChanged.connect(self._check_logic_states)
        btn_arxml = QPushButton("BROWSE")
        btn_arxml.setObjectName("util_btn")
        btn_arxml.clicked.connect(self._browse_arxml_file)
        inputs_vbox.addWidget(
            self._create_input_row(
                "Input Signal DB (ETH_CAN.arxml):", self.arxml_path_edit, btn_arxml
            )
        )

        self.aacp_path_edit = QLineEdit()
        self.aacp_path_edit.textChanged.connect(self._check_logic_states)
        btn_aacp = QPushButton("BROWSE")
        btn_aacp.setObjectName("util_btn")
        btn_aacp.clicked.connect(self._browse_aacp_file)
        inputs_vbox.addWidget(
            self._create_input_row(
                "Input AACP sysvar (aacp.vsysvar):", self.aacp_path_edit, btn_aacp
            )
        )

        self.someip_path_edit = QLineEdit()
        self.someip_path_edit.textChanged.connect(self._check_logic_states)
        btn_someip = QPushButton("BROWSE")
        btn_someip.setObjectName("util_btn")
        btn_someip.clicked.connect(self._browse_someip_file)
        inputs_vbox.addWidget(
            self._create_input_row(
                "Input SOMEIP_FF sysvar (SysVarDef.xml):", self.someip_path_edit, btn_someip
            )
        )
        ...
        self.btn_preprocess.setStyleSheet(
            "background-color: #3b3e40; color: #94a3b8; border-radius: 4px; border: none;"
        )
    ...
    def run_preprocess(self):
        if self.pre_process_done:
            reply = QMessageBox.question(
                self,
                "Pre-Process Again?",
                "Previous data exists. Re-process?",
        ...
        self.btn_preprocess.setEnabled(False)
        self.btn_preprocess.setStyleSheet(
            "background-color: #d68910; color: white; border-radius: 4px; border: none;"
        )
        self.pre_process_done = False
        self.btn_gen.setEnabled(False)
        self.btn_gen.setStyleSheet(
        ...
        def task():
            ...
            try:
                ...
                if not any([can_built, eth_built, aacp_built, someip_built]):
                    self.signals.log_signal.emit(
                        "⚡ Valid Database/Sysvar cache was found. Re-Parsing skipped."
                    )
                else:
                    if can_built or eth_built:
                        self.signals.log_signal.emit("✅ ARXML Databases parsed successfully.")
                    if aacp_built:
                        self.signals.log_signal.emit("✅ AACP System Variables processed.")
                    if someip_built:
                        self.signals.log_signal.emit("✅ SOMEIP_FF Definitions processed.")
                    self.signals.log_signal.emit(f"⏱️ Parsing Phase Time: {parse_time}")

                # --- PHASE 2: PRE-PROCESSING ---
                pre_start = time.time()
                self.signals.log_signal.emit(
                    "⚙️ Pre-Processing Input requirement specifications ..."
                )
                self.pipeline.run_preprocessing_memory(
                    self.req_path_edit.text(), "GeneratedTestScripts"
                )

                pre_time = self._format_time(time.time() - pre_start)
                self.signals.log_signal.emit(
                    f"✅ Requirements mapping complete. (Time: {pre_time})"
                )

                total_time = self._format_time(time.time() - total_start_time)
                self.signals.log_signal.emit(
                    f"🏁 Pre-Processing Complete. (Total Elapsed Time: {total_time})"
                )
    ...
    def _browse_file(self):
        f, _ = QFileDialog.getOpenFileName(
            self, "Select Requirements", "", "Excel Files (*.xlsx *.xls)"
        )
        if f:
            self.req_path_edit.setText(f)
            self.write_log(f"📄 Excel Selected: {os.path.basename(f)}")
    ...
    def _browse_aacp_file(self):
        f, _ = QFileDialog.getOpenFileName(
            self, "Select AACP Sysvar", "", "vsysvar Files (*.vsysvar)"
        )
        if f:
            if f != self.aacp_path_edit.text():
                self.pipeline.aacp_data = {}  # Assuming your pipeline tracks this cache
            self.aacp_path_edit.setText(f)
            self.write_log(f"🔗 AACP Selected: {os.path.basename(f)}")
    ...
    def _browse_someip_file(self):
        ...
        if f:
            ...
            self.someip_path_edit.setText(f)
            self.write_log(f"🔗 SOMEIP_FF Selected: {os.path.basename(f)}")
    ...
    def _build_header(self):
        ...
        if logo_path.exists():
            l_lbl = QLabel()
            l_pix = QPixmap(str(logo_path)).scaledToHeight(
                55, Qt.TransformationMode.SmoothTransformation
            )
    ...
    def run_generation(self):
        ...
        try:
            self.write_log(
                f"📋 Generating: {self.cat_combo.currentText()} / {self.typ_combo.currentText()}"
            )
            self.pipeline.run_generation(
    ...
    def download_logs(self):
        ...
        f, _ = QFileDialog.getSaveFileName(
            self, "Save Logs", f"CaplGenLogs_{ts}.txt", "Text Files (*.txt)"
        )
        if f:
