...
class CANMapper(BaseMapper):
    ...
    def get_signal_data(self, port_name: str, cluster: str) -> dict:
        ...
        for c in fallback_order:
            match = next(
                (p for p in paths if str(p.get("can_cluster")).upper() == str(c).upper()), None
            )
            if match:
    ...
    def resolve(self, df_subset: pd.DataFrame) -> pd.DataFrame:
        ...
        can1_results = pd.DataFrame(
            [
                self.get_signal_data(p, c)
                for p, c in zip(df_subset["CAN_PORT"], df_subset["CAN_CLUSTER"], strict=False)
            ],
        ...
        if is_can_to_can.any():
            ...
            can2_results = pd.DataFrame(
                [
                    self.get_signal_data(p, c)
                    for p, c in zip(
                        can2_subset["CAN_TO_CAN_MAPPING"], can2_subset["CAN2_CLUSTER"], strict=False
                    )
                ],
