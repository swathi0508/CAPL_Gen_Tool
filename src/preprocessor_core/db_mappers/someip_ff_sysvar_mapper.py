    ...
    def _load_database(self) -> dict:
        ...
        for iface_name, iface_data in interfaces.items():
            ...
            for node_name, node_data in nodes.items():
                ...
                ctrl_sig = next(
                    (c.get("Signal_DB_Name") for c in controls.values() if c.get("Signal_DB_Name")),
                    None,
                )
    ...
    def resolve(self, df_subset: pd.DataFrame) -> pd.DataFrame:
        ...
        def process_row(r):
            ...
            if "PIU_MASTER" in port_val.upper():
                port_val = port_val.replace("PIU_MASTER", "PIU_Mst").replace(
                    "Piu_Master", "PIU_Mst"
                )
            ...
            for data in self.raw_db_values:
                if (
                    data["iface_key"] == target_iface
                    and data["node_key"] == target_node
                    and data["role_key"] == target_role
                ):
            ...
            if sig_data:
                ...
                if "valuestate" in target_attr.lower():
                    ...
                    vs_candidates = [
                        f"{target_attr}ValueState".lower(),
                        f"{target_attr}value_state".lower(),
                        "valuestate",
                    ]
            ...
            for pre, db_col in [
                ("SIGNAME", "SOMEIP_FF_DB_SIGNAL_NAME"),
