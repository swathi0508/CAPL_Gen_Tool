...
class EnumResolver:
    ...
    def resolve_enum_mapping(self, df: pd.DataFrame, sig1_col: str, sig2_col: str) -> pd.DataFrame:
        ...
        res_cols = [
            "ENUM_LEXICAL_MATCH",
            f"COMPUTED_{p1}_VALUE_MIN",
            f"COMPUTED_{p1}_VALUE_MID",
            f"COMPUTED_{p1}_VALUE_MAX",
            f"COMPUTED_{p2}_VALUE_MIN",
            f"COMPUTED_{p2}_VALUE_MID",
            f"COMPUTED_{p2}_VALUE_MAX",
        ]
    ...
    def resolve_single_enum(self, df: pd.DataFrame, sig_enum_col: str) -> pd.DataFrame:
        ...
        res_cols = [
            f"COMPUTED_{prefix}_VALUE_MIN",
            f"COMPUTED_{prefix}_VALUE_MID",
            f"COMPUTED_{prefix}_VALUE_MAX",
        ]
    ...
    def _resolve_single_row_logic(self, row, prefix, col_name):
        ...
        filtered_keys = sorted(
            [
                ...
                if not self.is_strictly_excluded(v)
                and str(k).lstrip("-").replace(".", "", 1).isdigit()
            ],
    ...
    def _resolve_mapping_logic(self, row, p1, col1, p2, col2):
        ...
        if f1 and f2:
            context = (
                str(row.get("SOMEIP_TOPIC_ATTRIBUTE", ""))
                + " "
                + " ".join(map(str, f1.values()))
                + " "
                + " ".join(map(str, f2.values()))
            ).lower()
            ...
            valid_results = [
                {"id1": k, "id2": final_map[k]} for k in sorted_f1_keys if k in final_map
            ]

            if len(valid_results) >= 2:
                min1, mid1, max1 = self._pick_boundaries_from_list(
                    [v["id1"] for v in valid_results]
                )
                min2, mid2, max2 = self._pick_boundaries_from_list(
                    [v["id2"] for v in valid_results]
                )
                lex_match = "MATCH"
        ...
        if lex_match == "MISMATCH":
            if f1 and not f2:
                ...
                min1, mid1, max1 = (
                    b1[f"COMPUTED_{p1}_VALUE_MIN"],
                    b1[f"COMPUTED_{p1}_VALUE_MID"],
                    b1[f"COMPUTED_{p1}_VALUE_MAX"],
                )
                min2, mid2, max2 = min1, mid1, max1
            elif f2 and not f1:
                b2 = self._resolve_single_row_logic(row, p2, col2)
                min2, mid2, max2 = (
                    b2[f"COMPUTED_{p2}_VALUE_MIN"],
                    b2[f"COMPUTED_{p2}_VALUE_MID"],
                    b2[f"COMPUTED_{p2}_VALUE_MAX"],
                )
                min1, mid1, max1 = min2, mid2, max2

        res = {
            "ENUM_LEXICAL_MATCH": lex_match,
            f"COMPUTED_{p1}_VALUE_MIN": min1,
            f"COMPUTED_{p1}_VALUE_MID": mid1,
            f"COMPUTED_{p1}_VALUE_MAX": max1,
            f"COMPUTED_{p2}_VALUE_MIN": min2,
            f"COMPUTED_{p2}_VALUE_MID": mid2,
            f"COMPUTED_{p2}_VALUE_MAX": max2,
        }
        ...
        for suf in ["MIN", "MID", "MAX"]:
            ...
            if v1 in ["N/A", "", "nan"] and v2 not in ["N/A", "", "nan"]:
                res[k1] = v2
            elif v2 in ["N/A", "", "nan"] and v1 not in ["N/A", "", "nan"]:
                res[k2] = v1
    ...
    def _pick_boundaries_from_list(self, items):
        ...
        if n == 2:
            return items[0], items[1], items[1]  # Rule: mid == max
    ...
    def calculate_priority_score(self, id1, id2, val1, val2, is_binary, is_gear):
        ...
        if is_gear:
            ...
            for short, full in gear_map.items():
                if (
                    re.search(rf"^{short}_|\b{short}\b", v1_raw) or full in v1_raw
                ) and full in v2_raw:
        ...
        c_dir = (
            "b_to_m"
            if any(x in v1_raw for x in ["battery_to_motor", "battery_to_rear"])
            else "m_to_b"
            if any(x in v1_raw for x in ["motor_to_battery", "rear_motor_to_battery"])
            else "none"
        )
        s_dir = (
            "b_to_m"
            if "battery_to_motor" in v2_raw
            else "m_to_b"
            if "motor_to_battery" in v2_raw
            else "none"
        )
    ...
    def normalize_strict(self, text):
        t = (
            str(text)
            .lower()
            .replace("no_activated", "not_activated")
            .replace("noactivated", "notactivated")
        )
    ...
    def get_polarity(self, text):
        ...
        return (
            "NEG"
            if any(word in tokens for word in neg) or "not" in text_clean or "no" in text_clean
            else "POS"
        )
    ...
    def _get_keys_bounds(self, d):
        keys = sorted(
            [str(k) for k in d.keys() if str(k).lstrip("-").replace(".", "", 1).isdigit()],
            key=lambda x: int(float(x)),
        )
