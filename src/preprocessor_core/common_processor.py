...
class CommonProcessor:
    ...
    def _compute_bounds(min_in, max_in):
        ...
        try:
            ...
            return {"MIN": float(final_min), "MID": float(final_mid), "MAX": float(final_max)}
        ...
        try:
            ...
            mapping = {
                ...
                "ATTRIBUTE_VALUE": [
                    "ATTRIBUTE_VALUE",
                    "ATTRIBUTE VALUE",
                    "SOMEIP_ATTRIBUTE_VALUE_MAPPING",
                ],
                "CAN_PORT": ["CAN_PORT", "CAN_PORT_MAPPING", "PORT NAME"],
                "PATH_SYNTHESIS": [
                    "PATH_SYNTHESIS",
                    "CAN_PATH_SYNTHESIS_MAPPING",
                    "PATH SYNTHESIS",
                ],
    ...
    def define_additional_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        additional_cols = [
            ...
            "SOMEIP_OFFSET",
            "SOMEIP_RESOLUTION",
            "SOMEIP_FF_DB_SIGNAL_NAME",
            "SOMEIP_FF_DB_SIGNAL_VALUESTATE",
            "SOMEIP_FF_DB_SIGNAL_CONTROL",
            "SOMEIP_FF_ENUM",
            "SOMEIP_FF_DATATYPE",
            "SOMEIP_FF_SIGNAME_NAMESPACE",
            "SOMEIP_FF_SIGNAME_VARIABLE",
            "SOMEIP_FF_SIGVALUESTATE_NAMESPACE",
            "SOMEIP_FF_SIGVALUESTATE_VARIABLE",
            "SOMEIP_FF_CONTROL_NAMESPACE",
            "SOMEIP_FF_CONTROL_VARIABLE",
            "AACP_DB_SIGNAL_NAME",
            ...
            "COMPUTED_SOMEIP_FF_VALUE_MIN",
            "COMPUTED_SOMEIP_FF_VALUE_MID",
            "COMPUTED_SOMEIP_FF_VALUE_MAX",
    ...
    def derive_can_cluster(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if {"PATH_SYNTHESIS", "TEST_TYPE"}.issubset(df.columns):
            df["CAN_CLUSTER"] = df.apply(
                ...
                if "CAN" in str(r["TEST_TYPE"]).upper()
                else "",
                axis=1,
            )
        ...
        if {"CAN_PORT", "CAN_CLUSTER", "CAN_TO_CAN_MAPPING", "TEST_TYPE"}.issubset(df.columns):
            ...
            if is_can_to_can.any():
                clean_mappings = (
                    df.loc[is_can_to_can, "CAN_TO_CAN_MAPPING"]
                    .astype(str)
                    .str.strip()
                    .str.replace("\u200b", "")
                )
        ...
        for row_tt_raw, group_df in df[enum_mask].groupby("TEST_TYPE"):
            ...
            # --- BRANCH 2: Mapping tests ---
                ...
                if "CAN->CAN" in row_tt or "CAN-CAN" in row_tt:
                    temp_df = self.enum_mapper.resolve_enum_mapping(
                        temp_df, "CAN_ENUM", "CAN2_ENUM"
                    )
                elif "SOMEIP_FF" in row_tt:
                    temp_df = self.enum_mapper.resolve_enum_mapping(
                        temp_df, "CAN_ENUM", "SOMEIP_FF_ENUM"
                    )
                elif "AACP" in row_tt:
                    temp_df = self.enum_mapper.resolve_enum_mapping(
                        temp_df, "CAN_ENUM", "AACP_ENUM"
                    )
                elif "CAROS" in row_tt:
                    temp_df = self.enum_mapper.resolve_enum_mapping(
                        temp_df, "CAN_ENUM", "SOMEIP_ENUM"
                    )
