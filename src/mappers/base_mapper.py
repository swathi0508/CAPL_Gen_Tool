...
class BaseMapper:
    ...
    def normalize_attr(val) -> str:
        ...
        if val_str.lower() == "valuestate": return None
        clean_val = re.sub(r'valuestate$', '', val_str, flags=re.IGNORECASE).strip()
        if not clean_val or clean_val.lower() in ["nan", "unknown"]: return None
        return clean_val

    @staticmethod
    def extract_cluster(path_val) -> str:
        if pd.isna(path_val) or str(path_val).strip() == "": return ""
        ...
        if any(trig in str(path_val) for trig in ["PIU_Mst", "PIU_Sub", "PIU_Hood"]):
