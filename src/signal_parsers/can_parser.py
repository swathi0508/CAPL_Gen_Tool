...
class CANSignalParser(BaseParser):
    ...
    def parse(self) -> Dict[str, Any]:
        ...
        # 2. Cache PDU Groups
        for gp in root.xpath(
            "//as:I-SIGNAL-I-PDU-GROUP[as:COMMUNICATION-DIRECTION='OUT']", namespaces=_ns
        ):
            ...
            self._cluster_cache.append(
                {
                    ...
                    "trig_nodes": cluster.xpath(
                        ".//*[local-name()='PDU-TRIGGERING']", namespaces=_ns
                    ),
                }
            )
    ...
    def _calculate_pdu_paths(self, pdu_name: str) -> List[Dict]:
        ...
        for cluster in self._cluster_cache:
            ...
            tr_node = next(
                (
                    t
                    for t in cluster["trig_nodes"]
                    if pdu_name in (t.findtext(".//as:I-PDU-REF", namespaces=self.ns) or "")
                ),
                None,
            )
                ...
                for port in ecu["ports"]:
                    if c_name in port["text"] and pdu_name in port["text"]:
                        if port["dir"] == "OUT":
                            ...
                            rx_list.add(ecu["name"])
                        elif cluster["is_eth"] and tr_xml:
                            if f"/{ecu['name']}/" in tr_xml and tx == "Unknown/Gateway":
                                tx = ecu["name"]
                            else:
                                rx_list.add(ecu["name"])
            ...
            if tx in rx_list:
                rx_list.remove(tx)
            if tx == "Unknown/Gateway" and c_name == "CAN_FD_CHASSIS":
                tx = "PCU_LLCE_2"
    ...
    def _index_compu_methods(self, root):
        ...
        for cm in root.xpath("//as:COMPU-METHOD", namespaces=_ns):
            ...
            for s in cm.xpath(".//as:COMPU-SCALE", namespaces=_ns):
                ...
                v = s.findtext(".//as:V", namespaces=_ns) or s.findtext(
                    "as:LOWER-LIMIT", namespaces=_ns
                )
    ...
    def _index_signals(self, root):
        ...
        sys_sig_to_cm = {
            ss.findtext("as:SHORT-NAME", namespaces=_ns): (
                ss.findtext(
                    ".//as:COMPU-METHOD-REF",
                    namespaces=_ns,
                )
        ...
        for i_sig in root.xpath("//as:I-SIGNAL", namespaces=_ns):
            ...
            if not name or not name.startswith("I"):
                continue

            cm_ref = i_sig.findtext(
                ".//as:COMPU-METHOD-REF",
                namespaces=_ns,
            )

            system_signal_ref = i_sig.findtext(
                ".//as:SYSTEM-SIGNAL-REF",
                namespaces=_ns,
            )

            system_signal_key = system_signal_ref.split("/")[-1] if system_signal_ref else None

            cm_key = cm_ref.split("/")[-1] if cm_ref else sys_sig_to_cm.get(system_signal_key)

            base_type_ref = i_sig.findtext(
                ".//as:BASE-TYPE-REF",
                namespaces=_ns,
            )

            self.sig_attr_data[name] = {
                "datatype": (base_type_ref.split("/")[-1] if base_type_ref else ""),
                "db_attr": self.compu_map.get(cm_key, {}),
            }

    def _resolve_signals(self, signal_list: List[str]) -> Dict[str, Any]:
        results = {}
        _topo_cache = self._topology_caches
        for sig_name in signal_list:
            pdu_name = self.sig_to_pdu.get(sig_name)
            if not pdu_name:
                results[sig_name] = {
                    "Status": "Missing from Topology",
                    "Attributes": {},
                    "signal_paths": [],
                }
                continue
            ...
            results[sig_name] = {
                ...
                "signal_paths": [
                    {
                        **path,
                        "signal_name": f"{path['can_cluster']}::{path['tx']}::"
                        f"{pdu_name}::{sig_name}",
                    }
